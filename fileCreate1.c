 ...
 * I lernt that filedescriptor is alwys 3 as linux picks the smallest present
 *0,1,2 are for stdin,stdout and stderror
 */
int main(int argc, char **argv)
{
	 ...
	 */
	strncpy(file_name,argv[1],sizeof(file_name));//Copy th
	printf("\nsize of filename==%d\n",sizeof(file_name));
	...
	w = write(2,write_buffer,write_length);//Added this myself to check how stdout file descriptor works
	w = write(3,write_buffer,write_length);
